...
def first_form_with_field(form_page, field_name):
    """
    Pick the correct form when the page has multiple POST forms (e.g. layout
    search, per-method shipping POST forms).

    WebTest stores each <form> under integer keys 0..n and again under id="…"
    if present, so iterating .values() would visit the same Form twice.
    """
    ...
    raise AssertionError(
        "No form with field %r found (form indices: %s)"
        % (field_name, [k for k in forms if isinstance(k, int)])
    )
...
class CheckoutMixin(object):
    ...
    def enter_guest_details(self, email="guest@example.com"):
        ...
        if index_page.status_code == 200:
            form = first_form_with_field(index_page, "username")
            form["username"] = email
...
class ShippingMethodViewMixin(
    ...
    def test_shows_form_when_multiple_shipping_methods_available(self, mock_repo):
        ...
        response = first_form_with_field(form_page, "method_code").submit()
        self.assertRedirectsTo(response, "checkout:payment-method")
    ...
    def test_check_user_can_submit_only_valid_shipping_method(
        ...
        form_page = self.get(reverse("checkout:shipping-method"))
        # a malicious attempt?
        ship_form = first_form_with_field(form_page, "method_code")
        ship_form["method_code"].value = "super-free-shipping"
        response = ship_form.submit()
