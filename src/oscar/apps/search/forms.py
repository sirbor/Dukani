...
class BrowseCategoryForm(SearchForm):
    ...
    def search(self):
        ...
        if self.categories is not None:
            ...
            if category_ids:
                sqs = sqs.filter(category__in=category_ids)
        return sqs


class DepartmentBrowseCategoryForm(BrowseCategoryForm):
    """
    Main catalogue browse: one control — filter by genre department (no price/title sort).
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if "sort_by" in self.fields:
            del self.fields["sort_by"]
        ...
        for dept in resolve_book_department_categories():
            choices.append((str(dept.pk), dept.name))

        self.fields["department"] = forms.ChoiceField(
            label=_("Browse by department"),
            choices=choices,
            required=False,
            widget=forms.Select(
                attrs={
    ...
    def search(self):
        ...
        department_pk = None
        if self.is_valid():
            raw = self.cleaned_data.get("department") or ""
            if raw != "":
                department_pk = int(raw)

        if department_pk is not None:
            sqs = sqs.filter(category=department_pk)
        elif category_ids:
            sqs = sqs.filter(category__in=category_ids)

        return sqs.order_by("-date_created")
