...
class ProductCreateUpdateView(PartnerProductFilterMixin, generic.UpdateView):
    ...
    def forms_valid(self, form, formsets):
        ...
        # Reload from DB so the instance (and product.attr cache) match what was
        # committed, avoiding stale in-memory state when redirecting or when
        # comparing to direct DB edits.
...
class CategoryListView(PublicVisibilityUpdateMixin, SingleTableView):
    ...
    def get_queryset(self):
        ...
        if books is not None:
            # Bookstore: dashboard categories root is the Books tree (then ten departments).
            return Category.objects.filter(pk=books.pk)
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ctx["form"] = self.form
        books = resolve_nav_books_category()
        if books is not None and not self.request.GET.get("name"):
            ctx["child_categories"] = Category.objects.filter(pk=books.pk)
        else:
...
class CategoryDetailListView(
    ...
    def get_table_data(self):
        books = resolve_nav_books_category()
        if books is not None and self.object.pk == books.pk:
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        ctx["child_categories"] = self.get_table_data()
