...
class DepartmentCategoryChoiceField(forms.ModelChoiceField):
...
class ProductCategoryForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        from oscar.apps.catalogue.category_resolution import resolve_nav_books_category

        books = resolve_nav_books_category()
        cid = self.instance.category_id if self.instance.pk else None

        # Any public category under the resolved Books root (not only the ten canonical
        # shelves), so staff can move a product to another shelf or subcategory.
        if books:
            in_tree = books.get_descendants_and_self().filter(is_public=True)
            pks = list(in_tree.values_list("pk", flat=True))
            if cid and cid not in pks:
                pks.append(cid)
            qs = Category.objects.filter(pk__in=pks)
            if cid:
                qs = qs.filter(Q(is_public=True) | Q(pk=cid))
            else:
                qs = qs.filter(is_public=True)
            qs = qs.order_by("path")
            self.fields["category"] = DepartmentCategoryChoiceField(
                queryset=qs,
                required=self.fields["category"].required,
                label=_("Department"),
                help_text=_(
                    "Shelf or subcategory under Books. Pick “Books” only if the title is "
                    "not yet under a specific shelf."
                ),
            )
